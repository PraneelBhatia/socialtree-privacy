...
**Subprocessors**: The two Google services involved — Firebase AI Logic and Gemini — are both operated by Google LLC and are the only external subprocessors that receive **user content**. SocialTree also sends **anonymous usage analytics** to **Google Analytics for Firebase** (Firebase Analytics, operated by Google LLC): event names such as "capture completed" or "paywall shown", small non-identifying event details (counts, categories, coarse duration buckets), and general device statistics (app version, iOS version, device model, language). These signals never include names, note text, transcripts, or any other user content. The app never sets a user ID and uses the analytics SDK variant without advertising-identifier support — no IDFA, no IDFV, no hardware identifier; the only identifier is a random per-install app-instance ID generated by the SDK. Analytics data is retained by Google Analytics for up to 14 months, is not linked to your identity, and can be disabled at any time in **Settings** ("Share Anonymous Usage Analytics"). Google's privacy policy: <https://policies.google.com/privacy>. No crash reporters or ad networks are used.
...
- No crash reporters, advertising networks, IDFA, fingerprinting, or hardware identifiers. Firebase Analytics receives only the anonymous, opt-out usage analytics and random per-install identifier described above; it is not used for advertising tracking.
- No data brokers
- No sale or sharing of personal data for advertising. Capture content is sent only to Google under your AI choice. Optional shared-link lookup contacts the profile site itself, and Google Analytics for Firebase receives only the anonymous usage statistics described above.
...
- **Turn analytics off:** Settings → "Share Anonymous Usage Analytics" (stops all analytics signals immediately)
